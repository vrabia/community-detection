...
def community_detection(nodes, edges, population=300, generation=30, r=1.5, file_name="ga_community_detection"):
    graph = nx.Graph()
    ...
    clustering = NodeClustering(list(result), graph)

    colors = plot_network(graph, clustering, False, False, f"images/{file_name}.png")
...
def generate_chrom(nodes_length, Adj):
    ...
    for x in range(nodes_length):
        ...
        count = 0
        while Adj[x, rand] != 1 and count < 100:
            rand = np.random.randint(0, nodes_length)
            count += 1
